...
- `POST /api/optimize` принимает фильтры и непустой объект `targets` с обязательными значениями. Незаданные характеристики не участвуют в поиске; подходящие сборки упорядочиваются по цене.
