...
- `simulator` — отдельный микросервис под будущую функциональность симулятора.
...
Скрипт создаст локальный `.env`, настроит Git hook и запустит весь стек с автоматическим HTTPS. После этого обновление выполняется обычной командой:
...
- `GET /api/simulator/health` проверяет контракт нового сервиса; логика симулятора пока не реализована.
