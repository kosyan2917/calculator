...
- `POST /api/optimize` принимает фильтры и непустой объект `targets` с обязательными значениями. Незаданные характеристики не становятся пользовательскими приоритетами. Новый генератор строит несколько промежуточных ограничений между ценовым, скоростным и защитным краями, сохраняет альтернативные составы около каждой точки и возвращает разнообразное множество решений с точной проверкой механик.
